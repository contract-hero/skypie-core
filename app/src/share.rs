...
// `skypie://` pairing / beam tickets). They share the picker and the anchor;
...
///
/// `deeplink::parse` owns the question "is this a link a Sky Pie can
...
mod macos {
    ...
    /// recipient gets something tappable that the `skypie://` handler opens.
...
/// Share a `skypie://` link (a pairing or beam ticket) via the native macOS
...
mod tests {
    ...
    fn accepts_the_two_link_shapes_the_ui_mints() {
        ...
        let pair = "skypie://pair?ticket=ABC";
        let receive = "skypie://receive?ticket=ABC&name=report.html&size=12";
    ...
    fn trims_surrounding_whitespace() {
        assert_eq!(validate_link("  skypie://pair?ticket=ABC\n").unwrap(), "skypie://pair?ticket=ABC");
    }
    ...
    fn rejects_the_verbs_that_carry_a_local_path() {
        ...
        for link in ["skypie://open?path=/etc/passwd", "skypie://reveal?path=/etc", "skypie://beam?path=/etc/passwd"] {
            assert!(validate_link(link).is_err(), "{link} must not reach the share sheet");
        }
    }
    ...
    fn rejects_a_bare_scheme_and_a_ticketless_verb() {
        assert!(validate_link("skypie://").is_err());
        assert!(validate_link("skypie://pair").is_err());
        assert!(validate_link("skypie://receive?t=ABC").is_err());
    }
    ...
    fn rejects_whitespace_and_control_characters_inside_a_ticket() {
        ...
        assert!(validate_link("skypie://pair?ticket=A B").is_err());
        assert!(validate_link("skypie://pair?ticket=A\nB").is_err());
    }
}
