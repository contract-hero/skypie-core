...
use skypie_app::remote::{beam, endpoint, Dirs};
use skypie_app::security::RootSet;
...
async fn beam_round_trip_then_stop_revokes() {
    ...
    assert!(offer.link.starts_with("skypie://receive?ticket="));
    ...
    match skypie_app::deeplink::parse(&offer.link).expect("own link re-parses") {
        skypie_app::deeplink::DeepLinkIntent::Receive { ticket, name, size } => {
