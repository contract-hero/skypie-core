// Where the remote subsystem keeps its files, the ignore list it shares with
// the local quick-open walker, and the small file primitives every module
// here reads and writes through.
//
// The crate hardcodes NO absolute directory. The desktop app passes
// `~/Library/Application Support/SkyPie`; a headless consumer (the MCP server)
...
/// Default-ignored directory names. Owned here and re-exported by the app's
/// scanner so one list covers every walk; nothing on the wire consults it
/// since the remote browse tree went away.
...
impl Dirs {
    ...
    /// `~/Library/Application Support/SkyPie`, a test's tempdir, or whatever a
    /// headless host owns.
    ...
    /// `<base>/remote/cache/` — artifacts pulled from a paired peer, named by
    /// content address.
    pub fn cache(&self) -> PathBuf {
        self.remote().join("cache")
    }

    /// `<base>/received/` — landed beams. Inside the consumer's own
...
/// wants an empty name, an offer wants "artifact", a received file wants the
...
/// store — which reaches this through `write_private_atomic` — are both "this
/// file IS a capability" documents, and a second writer is how one of them
/// silently loses its mode.
...
/// for this crate's 0600 documents, because a half-written capability
/// document is a peer list missing the machine that was just paired.
///
/// The parent directory is created here rather than at the call site, so a
/// write after the state directory was cleared out from under a running app
/// restores the file instead of failing.
