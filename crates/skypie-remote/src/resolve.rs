// Turning "forget my old iPad" into exactly one paired peer — and, on the
// same machinery, "confirm THAT pairing" into exactly one parked pairing, and
// a hash prefix into exactly the beam links it names.
//
// The caller is a language model (through the MCP server) with a free-text
// name, so the match is deliberately forgiving — full node id, device name, a
// prefix of either — and deliberately refuses to guess: two candidates is an
// error that lists both, never the first one sorted. Unpairing the wrong
// machine destroys a working pairing, and confirming the wrong pairing writes
// trust to disk.
//
...
use crate::peers::{short_id, Peer, PendingPair};
