...
/// that a link went dead at once, and an expired grant unpins an artifact a
/// peer asked for. One minute keeps that window short enough to match what
/// `stop_beam` promises, and a mark-and-sweep over a store holding a handful
/// of artifacts is cheap.
///
...
///
/// Every Claude Code session spawns its own `skypie-mcp` against one state
...
impl StoreLock {
    ...
    fn acquire(dirs: &Dirs) -> Result<Self, String> {
        ...
        // The app is the only process that opens the store, so the other
        // holder is a second copy of the app (or a dev build beside the
        // installed one) over the same `SKYPIE_STATE_DIR`.
        file.try_lock().map_err(|e| match e {
            std::fs::TryLockError::WouldBlock => format!(
                "another Sky Pie is already using the blob store at {path:?}. \
                 Only one can run per state directory — close the other one."
            ),
            ...
            std::fs::TryLockError::Error(e) if e.kind() == std::io::ErrorKind::Unsupported => {
                format!(
                    "this filesystem cannot lock files, so SkyPie cannot be sure it is the \
...
pub async fn boot_with_gc(
    ...
    // back: a revoked beam link and an expired grant each deleted their tag
    // and left a private copy of somebody's file in `remote/blobs/` for as
    // long as the state directory lived.
    //
    ...
    let store = FsStore::load_with_opts(
        ...
        Options {
            ...
            // holds while it runs, so an offer's `auto-…` tag and a grant's
            // `auto-…` tag both keep their bytes, and bytes no tag names are
            // exactly the bytes nothing can reach — the request gate admits a
            // hash only through `Offers` or `Grants`, and both are built from
            // a tag.
    ...
    if released > 0 {
        eprintln!("skypie: remote: released {released} staged file(s) left by an earlier run");
    }
    ...
    // carries content, `skypie/scope/1` carries the session, `skypie/pair/1` is
    ...
    let scope_server = scope_state.map(|state| {
        let server = Arc::new(scope::ScopeServer::new(state.clone(), store.clone(), grants.clone()));
        (server, state)
    });
...
mod tests {
    ...
    fn second_claim_on_one_store_is_refused_not_queued() {
        ...
        assert!(
            msg.contains("close the other one"),
