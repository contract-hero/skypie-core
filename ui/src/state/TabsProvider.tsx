...
import type { IpcSurface, LoadErrorKind, RemoteGetError } from "../ipc";
...
export function TabsProvider({ ipc, children }: TabsProviderProps): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    ipc
      ...
      .catch((e: unknown) => {
        if (loadSeq.current !== seq) return;
        // Tauri rejects with the command's `Err` value. `read_file` is
        // declared `Result<FilePayload, String>`, so a local read rejects
        // with a plain string; a pull (`remote_get`) rejects with a typed
        // `{ cause, reason }`, and the cause is what the tab decides on.
        const error = loadErrorOf(path, e);
        dispatch({ type: "LOAD_ERROR", tabId, path, nonce: reloadNonce, error });
      });
  ...
  // A pulled tab is external too, but its address is a `skypie-remote://`
  // string, not a local path, so it never goes to `watch_external_paths`.
  ...
  React.useEffect(() => {
    ...
    void ipc.watchExternalPaths(externalPaths).catch((e: unknown) => {
      console.warn("skypie: failed to update external file watches", e);
    });
...
/** The reducer's error shape from whatever the IPC rejected with. A pull
 *  failure carries its cause as `kind: "remote-<cause>"`. */
function loadErrorOf(
  path: string,
  e: unknown,
): { kind: LoadErrorKind; path: string; reason: string } {
  if (isRemoteAddress(path) && typeof e === "object" && e !== null && "cause" in e) {
    const { cause, reason } = e as RemoteGetError;
    return { kind: `remote-${cause}`, path, reason };
  }
  const reason = e instanceof Error ? e.message : String(e);
  return { kind: isRemoteAddress(path) ? "remote-unknown" : "Io", path, reason };
}
  ...
  return React.useCallback(
    (path: string, opts?: OpenFileOptions) => {
      ...
      void ipc.pushRecent?.(path)
        .then(() => refreshRecents())
        .catch((e: unknown) => console.error("skypie: failed to record recent", path, e));
    },
