...
// provider only sequences IPC commands and `skypie://beam-*` events.
//
...
// acted — offer immediately) and the `skypie://beam` deep link (a link is
...
/** Payload of `skypie://beam-send-request` (CLI / deep-link initiated). */
...
/** Payload of `skypie://beam-receive-request`. `name` arrives pre-sanitized
...
export function BeamProvider({
  ...
  useTauriEvent<BeamSendRequestPayload>("skypie://beam-send-request", (req) => {
    setSend({ path: req.path, name: req.name, size: req.size, phase: "confirm" });
  });

  useTauriEvent<BeamReceiveRequestPayload>("skypie://beam-receive-request", (req) => {
  ...
  useTauriEvent<BeamProgressPayload>("skypie://beam-progress", (p) => {
    setReceive((r) =>
  ...
  useTauriEvent<BeamOffer[]>("skypie://beam-offers-updated", (list) => {
    setOffers(list);
  });
  ...
  const refreshReceived = React.useCallback(() => {
    ipc.beamListReceived?.()
      ...
      .catch((e: unknown) => {
        console.error("skypie: beam received list failed", e);
  ...
  const stopOffer = React.useCallback(
    (id: string) => {
      ...
      ipc.beamStop?.(id).catch((e: unknown) => {
        console.error("skypie: beam stop failed", id, e);
