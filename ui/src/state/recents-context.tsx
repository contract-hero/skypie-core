...
export function RecentsProvider({
  ...
  const refresh = React.useCallback(() => {
    ...
    ipc.listRecents().then(setRecents).catch((e: unknown) => {
      ...
      console.error("skypie: failed to list recents", e);
    });
  }, [ipc]);
  ...
  // No backend emits `skypie://recents-updated` today; the subscription is
  // kept for forward compat with a backend that broadcasts recents updates.
  useTauriEvent<RecentEntry[]>("skypie://recents-updated", setRecents);
