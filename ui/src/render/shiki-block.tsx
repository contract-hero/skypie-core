...
export default function ShikiBlock({ code, lang }: ShikiBlockProps): React.ReactElement {
  ...
  // The fallback is thousands of elements on a large file, and it is rebuilt
  // on every render of this component — the `setReady` commit, a theme
  // change, any parent re-render — even though it only depends on the code.
  const fallback = React.useMemo(() => lineSpans(code), [code]);
  ...
  return (
    <div data-shiki-block data-shiki-ready={ready ? "true" : undefined} className="shiki-block">
      ...
      <div ref={containerRef}>
        <pre>
          <code className={`language-${lang}`}>{fallback}</code>
        </pre>
      </div>
    </div>
  );
