...
describe("formatRemoteAddress / parseRemoteAddress", () => {
  it("round-trips a peer id and an absolute path", () => {
    ...
    expect(address).toBe("skypie-remote://abcd1234/w/project/report.html");
  ...
  it("recognizes the scheme", () => {
    expect(isRemoteAddress("skypie-remote://peer/a.md")).toBe(true);
  ...
  it("rejects non-remote addresses", () => {
    ...
    expect(parseRemoteAddress("skypie://open?path=/a")).toBeNull();
  });

  it("rejects a malformed remote address (no peer, or a path missing its slash)", () => {
    expect(parseRemoteAddress("skypie-remote:///a.md")).toBeNull();
    expect(parseRemoteAddress("skypie-remote://peer")).toBeNull();
    expect(parseRemoteAddress("skypie-remote://peer/")).toEqual({ peer: "peer", path: "/" });
  });
});
