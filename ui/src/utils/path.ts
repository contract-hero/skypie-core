 ...
 * `/w/skypie` would match `/w/skypie-worktrees/x` and render a relative path
 ...
 *  - `skypie://open?path=…` / `skypie://reveal?path=…` — the `path` param is
...
export function normalizePathBarInput(raw: string): PathBarNormalization {
  ...
  if (candidate.startsWith("skypie://")) {
    const queryIndex = candidate.indexOf("?");
    if (queryIndex === -1) {
      return { kind: "error", reason: "skypie:// URL is missing a path" };
    }
    ...
    if (!rawPath) return { kind: "error", reason: "skypie:// URL has no path= parameter" };
