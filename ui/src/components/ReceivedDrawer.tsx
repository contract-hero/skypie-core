...
export default function ReceivedDrawer({ onOpen }: ReceivedDrawerProps = {}): React.ReactElement {
  ...
  return (
    <SidebarSection id="received" title="Received" count={received.length}>
      {/* A refresh that failed leaves the list stale rather than empty,
          which is indistinguishable from "nothing arrived" unless it says
          so. */}
      {receivedError ? (
      ...
      {received.length === 0 ? (
        <p className="section-empty-hint">
          Files beamed to this device show up here.
        </p>
      ) : (
        <ul>
          {received.map((entry) => (
