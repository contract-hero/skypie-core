...
function SendFace({ ipc }: { ipc: IpcSurface }): React.ReactElement {
        ...
        {send.phase === "confirm" ? (
          <>
            <p className="beam-hint">
              Creates a link anyone with Sky Pie can fetch this file
              with — peer-to-peer, end-to-end encrypted, no account needed.
              Anyone holding the link can fetch it while the beam is active.
            </p>
        ...
        {send.phase === "ready" && send.offer ? (
          <>
            ...
            <div className="beam-actions">
              ...
              <ShareLinkButton ipc={ipc} link={send.offer.link} sheetTitle="Sky Pie beam link" />
...
export default function BeamDialog({ ipc }: { ipc: IpcSurface }): React.ReactElement | null {
  const { send, receive } = useBeamState();
  if (!send && !receive) return null;
  return (
    <div className="beam-backdrop" data-testid="beam-backdrop">
      {receive ? <ReceiveFace /> : <SendFace ipc={ipc} />}
    </div>
  );
}
