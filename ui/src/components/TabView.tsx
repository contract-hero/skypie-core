...
import { isRefusedKind, isUnpairedKind, readErrorTitle } from "../utils/read-error";
...
export default function TabView({
  ...
  if (isErrorPayload(tab.payload)) {
    const { path, reason } = tab.payload.error;
    // The reducer stamps `LoadErrorKind`; the payload union only knows it as
    // a string.
    const kind = tab.payload.error.kind as LoadErrorKind;
    // A pulled file that did not arrive: the device it lives on is what the
    // reader can act on, so the notice names it and offers the two moves
    // that help — try again once it is awake, or pair it.
    const remote = parseRemoteAddress(path);
    const device = remote ? deviceLabel(remote.peer) : null;
    const unpaired = remote != null && isUnpairedKind(kind);
    const title = readErrorTitle(kind, reason);
    const hint = unpaired
      ? "This link points to a device this app isn't paired with. Pair it under Settings → Devices, then try again."
      : remote && isRefusedKind(kind)
        ? `${device} answered and refused: ${reason}`
        : remote
          ? `Sky Pie on ${device} must be open and online to fetch this file. Try again once it is.`
          : reason;
    return (
      <div role="alert" data-testid="preview-error" className="file-notice">
        <FileWarning size={28} strokeWidth={1.5} aria-hidden />
        <p className="file-notice-title">{title}</p>
        <p className="file-notice-path">{remote ? `${device}: ${remote.path}` : path}</p>
        <p className="file-notice-hint">{hint}</p>
        <div className="file-notice-actions">
          <button
            type="button"
            className="button"
