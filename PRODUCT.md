...
which is a growing population and the reason the pivot is worth making. What
this costs the design: onboarding now matters for someone who has never seen
the app, and error text is read by strangers rather than by the author.

Two founding principles bend as a result, and both bend narrowly:

- **"Read, do not edit" still holds for the user's tree.** Comments are writes,
  but they land in the app's state directory, never beside the file. The one
  action that writes into the user's tree — "Export feedback next to file" —
  is explicit and never automatic.
- **"A paired peer can never write" becomes "a paired peer may append
  annotations".** Bounded: paired devices only, schema-validated, size-capped,
  append-only, and landing in the state directory — the same rule received
  files already follow.

## Product Purpose

Sky Pie is a distraction-free reading room for local HTML and Markdown
...
something Sky Pie should have done.

## Positioning

Four properties together, which no neighboring tool has:

1. **Deep-link addressable.** The `skypie://open?path=…` URL scheme makes any
   ...
   `skypie://` link by user convention.
...
4. **Peer-to-peer artifact sharing.** Pair your own devices once, then a
   `skypie://open?…&from=` link opens a file on any of them, pulled straight
   from the machine it lives on; for anyone else, **Beam** serves one artifact
   behind a `skypie://receive` link — direct, end-to-end encrypted, no VPN and
   no upload. This is the remote foundation (iroh) the iOS companion pairs
   over, and the seam through which the three properties above extend across
   machines (see
...
- **Trigger sources.** Files arrive from Claude Code sessions, the `skypie` CLI
  binary, a `⌘O` file picker, the workspace tree, pasted paths, and — over the
  network — an accepted Beam from another instance.
- **Companion surfaces.** A `skypie` CLI (`skypie open`, `skypie reveal`,
  `skypie beam`, `skypie receive`), a Slack hand-off path (native share sheet
  plus an Open-in-Slack deep link), device links for the user's own paired
  devices, and Beam — one-link peer-to-peer artifact transfer to anyone. A
  stdio MCP server, `skypie-mcp`, is the agent-integration surface: a thin
  client of the running app that lets Claude Code and other agentic tools
  mint a device link, beam an artifact, pair, and list paired devices without
...
address bar accepting absolute paths, `file://` and `skypie://`; quick open
...
Binding: the product name **Sky Pie**, the `skypie://` URL scheme, and the
`skypie` CLI name.
